    ...
    >
        <div className="flex justify-between items-start mb-6">
            <div className="flex gap-4">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-[#8B5CF6]/20 to-indigo-500/20 border border-white/10 flex items-center justify-center text-xl font-bold text-white shadow-xl shadow-black/20 group-hover:scale-105 transition-transform">
                    {mentee?.startupName?.[0] || '?'}
                </div>
                <div>
                    <h4 className="text-white font-black group-hover:text-[#8B5CF6] transition-colors">{mentee?.startupName || 'Unknown Startup'}</h4>
                    <p className="text-gray-500 text-xs font-bold uppercase tracking-widest">{mentee?.founderName || 'Founder'}</p>
                </div>
            </div>
            <div className={`p-2 rounded-lg bg-green-500/10 text-green-400 border border-green-500/20 flex items-center gap-1`}>
                <TrendingUp size={14} />
                <span className="text-[10px] font-black uppercase">{mentee.growth || '8%'}</span>
            </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="p-3 bg-white/5 rounded-xl border border-white/5">
                ...
                <p className="text-xs font-bold text-gray-300">{mentee.lastSession || 'Oct 12'}</p>
            </div>
            <div className="p-3 bg-[#8B5CF6]/5 rounded-xl border border-[#8B5CF6]/10">
                ...
                <p className="text-xs font-bold text-white">{mentee.nextSession || 'In 4h'}</p>
            </div>
        </div>
                ...
                <div className="bg-[#1E1E2F] p-8 rounded-3xl border border-white/5 shadow-2xl relative overflow-hidden">
                    ...
                    <div className="text-center">
                        ...
                        <div className="flex flex-wrap justify-center gap-2">
                            <span className="px-3 py-1 bg-white/5 rounded-full text-[10px] font-black uppercase text-gray-400">{mentee.sector}</span>
                            <span className="px-3 py-1 bg-white/5 rounded-full text-[10px] font-black uppercase text-gray-400">Vanguard Mentee</span>
                        </div>
                    </div>
                    ...
                    <div className="mt-10 pt-10 border-t border-white/5 space-y-6">
                        ...
                        <div>
                            ...
                            <p className="text-white font-bold">{new Date(mentee.joinedAt || Date.now()).toLocaleDateString()}</p>
                        </div>
                    </div>
                </div>

                <div className="p-6 bg-gradient-to-br from-[#8B5CF6] to-indigo-600 rounded-3xl text-white shadow-xl shadow-[#8B5CF6]/20">
                    <h4 className="font-black text-sm mb-4 uppercase tracking-wider">Quick Action</h4>
                    <p className="text-white/80 text-xs font-medium leading-relaxed mb-6">Send a quick nudge or feedback directly to the founder's workspace.</p>
            ...
            <div className="lg:col-span-3 space-y-10">
                ...
                <div className="grid grid-cols-3 gap-6">
                    <div className="bg-[#1E1E2F] p-6 rounded-2xl border border-white/5">
                        <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Execution Score</p>
                        <p className="text-2xl font-black text-white">{mentee.executionScore || 78} <span className="text-green-400 text-xs">↑ 4%</span></p>
                    </div>
                    <div className="bg-[#1E1E2F] p-6 rounded-2xl border border-white/5">
                        <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Traction</p>
                        <p className="text-xl font-black text-white text-ellipsis overflow-hidden whitespace-nowrap">{mentee.traction || 'Pilot'}</p>
                    </div>
                    <div className="bg-[#1E1E2F] p-6 rounded-2xl border border-white/5">
                        <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Growth</p>
                        <p className="text-2xl font-black text-white">12.5% <span className="text-green-400 text-xs">↑</span></p>
                    </div>
                </div>
                ...
                <div className="bg-[#1E1E2F] p-8 rounded-3xl border border-white/5">
                    ...
                    <div className="space-y-4">
                        {[
                            { label: 'Core Problem Validation', completed: true },
                            { label: 'Initial GTM Strategy Design', completed: true },
                            { label: 'Technical Prototype Implementation', completed: false },
                            { label: 'Seed Round Preparation', completed: false },
                        ].map((item, i) => (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <section className="space-y-6">
                        ...
                        <div className="p-6 bg-[#0F0F14] rounded-2xl border border-white/5">
                            <p className="text-sm font-medium text-gray-400 leading-relaxed italic">
                                "Double down on the enterprise sales funnel. The current self-serve model is leaking high-value leads. Focus on high-touch closing for the next 3 months."
                            </p>
                            <p className="text-[10px] font-black text-gray-600 mt-4 uppercase tracking-widest text-right">— Last Session Notes</p>
                        </div>
                    </section>
                    ...
                    <section className="space-y-6">
                        ...
                        <div className="space-y-3">
                            {['Enterprise Outreach', 'Team Structure', 'Product Roadmap'].map((item, i) => (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {mentees.length === 0 ? (
                            <div className="col-span-full py-20 bg-[#1E1E2F] rounded-3xl border border-dashed border-white/10 flex flex-col items-center justify-center text-center">
                                ...
                                <p className="text-gray-600">Accept requests to start your mentorship journey.</p>
                            </div>
                        ) : (
                            mentees.map((mentee) => (
                                <MenteeCard
