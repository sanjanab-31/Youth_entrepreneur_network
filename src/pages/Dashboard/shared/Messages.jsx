...
const Messages = () => {
    ...
    return (
        <div className="h-[calc(100vh-160px)] flex flex-col animate-in fade-in duration-500">
            ...
            <div className="flex-1 bg-[#1E1E2F] rounded-3xl border border-white/5 overflow-hidden flex shadow-2xl">
                {/* Chat Sidebar */}
                <aside className="w-full md:w-80 border-r border-white/5 flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-1 p-2">
                        {chats[activeTab].map((chat) => (
                            <div key={chat.id} className={`flex items-center gap-4 p-4 rounded-2xl cursor-pointer transition-all group ${chat.id === 1 ? 'bg-[#8B5CF6]/10 border border-[#8B5CF6]/20' : 'hover:bg-white/5 border border-transparent'}`}>
                ...
                {/* Chat Main Area */}
                <main className="hidden md:flex flex-1 flex-col bg-[#0F0F14]/20 backdrop-blur-sm">
                    {/* Chat Header */}
                    <header className="p-6 border-b border-white/5 flex justify-between items-center bg-[#1E1E2F]/50">
                        <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center font-black text-gray-400">SS</div>
                            <div>
                                <h3 className="text-sm font-black text-white">Siddharth Sharma</h3>
                                <p className="text-[10px] text-green-500 font-bold uppercase tracking-widest">Active Now</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-4">
                            <button className="text-gray-600 hover:text-white transition-colors"><Phone size={20} /></button>
                            <button className="text-gray-600 hover:text-white transition-colors"><Video size={20} /></button>
                            <span className="w-[1px] h-6 bg-white/5" />
                            <button className="text-gray-600 hover:text-white transition-colors"><Info size={20} /></button>
                        </div>
                    </header>

                    {/* Message Area */}
                    <div className="flex-1 p-8 overflow-y-auto space-y-8 flex flex-col justify-end">
                        ...
                        <div className="flex gap-4 max-w-lg">
                            <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-[10px] font-black text-white flex-shrink-0">SS</div>
                    ...
                    <footer className="p-6 pt-0">
                        <div className="bg-[#0F0F14] border border-white/10 rounded-2xl p-4 flex items-end gap-4 shadow-2xl">
                            <button className="p-2 text-gray-600 hover:text-blue-400 transition-colors"><Paperclip size={20} /></button>
                            <textarea
