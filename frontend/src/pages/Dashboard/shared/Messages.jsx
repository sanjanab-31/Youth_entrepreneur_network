...
const Messages = () => {
    ...
    }, [conversationMessages, selectedConvo]);

    useEffect(() => {
        if (selectedConvo) {
            void loadConversationMessages(selectedConvo);
            void markAsRead(selectedConvo);
        }
    }, [selectedConvo, loadConversationMessages, markAsRead]);
    ...
    const activeMessages = selectedConvo ? conversationMessages : [];
            ...
            <div className="flex-1 bg-[#1E1E2F] rounded-[2.5rem] border border-white/5 overflow-hidden flex shadow-2xl relative">
                ...
                <aside className={`w-full md:w-80 border-r border-white/5 flex flex-col bg-[#15151e]/50 ${selectedConvo ? 'hidden md:flex' : 'flex'}`}>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-2">
                        {error && (
                            <div className="px-3 py-2 bg-red-500/10 border border-red-500/20 rounded-xl text-red-300 text-[10px] font-bold uppercase tracking-widest">
                                {error}
                            </div>
                        )}
                ...
                <main className={`flex-1 flex flex-col bg-[#0F0F14]/20 ${!selectedConvo ? 'hidden md:flex' : 'flex'}`}>
                    {!selectedConvo ? (
                        ...
                        <>
                            ...
                            >
                                {conversationError && (
                                    <div className="px-4 py-2 bg-red-500/10 border border-red-500/20 rounded-xl text-red-300 text-[10px] font-bold uppercase tracking-widest">
                                        {conversationError}
                                    </div>
                                )}
                                {conversationLoading && (
                                    <div className="px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-gray-400 text-[10px] font-bold uppercase tracking-widest">
                                        Loading messages...
                                    </div>
                                )}
